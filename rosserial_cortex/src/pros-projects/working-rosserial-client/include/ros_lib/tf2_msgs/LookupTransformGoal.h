...
namespace tf2_msgs
{
  ...
  class LookupTransformGoal : public ros::Msg
  {
    ...
    {
      int offset = 0;
      uint32_t length_target_frame = vexstrlen(this->target_frame);
      ...
      uint32_t length_source_frame = vexstrlen(this->source_frame);
      ...
      *(outbuffer + offset + 3) = (this->target_time.nsec >> (8 * 3)) & 0xFF;
      ...
      uint32_t length_fixed_frame = vexstrlen(this->fixed_frame);
