  ...
  class Log : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (u_level.base >> (8 * 0)) & 0xFF;
      ...
      offset += length_name;
      uint32_t length_msg = vexstrlen(this->msg);
      varToArr(outbuffer + offset, length_msg);
      offset += 4;
      memcpy(outbuffer + offset, this->msg, length_msg);
      offset += length_msg;
      uint32_t length_file = vexstrlen(this->file);
      varToArr(outbuffer + offset, length_file);
      offset += 4;
      memcpy(outbuffer + offset, this->file, length_file);
      offset += length_file;
      uint32_t length_function = vexstrlen(this->function);
      ...
      *(outbuffer + offset + 3) = (this->topics_length >> (8 * 3)) & 0xFF;
      ...
      uint32_t length_topicsi = vexstrlen(this->topics[i]);
